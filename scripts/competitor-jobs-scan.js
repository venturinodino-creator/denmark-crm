...
// listed — the feed is for what could be filled from Denmark. The other two
...
// in the tracked categories based in Denmark or remote, and every URL it
...
Include a role only if it is based in Denmark (Copenhagen, Aarhus, Odense, Aalborg) or is a remote role — remote from anywhere counts, including "US - Remote".
Start from the careers page ${careersUrl} and the company's LinkedIn jobs page; also search "${company} jobs Denmark", "${company} remote jobs" and the category names with "${company}". Ignore other companies with similar names, closed or expired postings, and roles outside the categories (engineering, product, editorial, finance, HR).
...
async function main() {
  ...
  for (const src of allSources) {
    ...
    try {
      ...
      { const c = perCompanyCounts[src.company]; console.log(`[competitor-jobs] ${src.company}: ${jobs.length} open role(s) — tracked categories: ${c.dkOrRemote} Denmark/remote listed; ${c.europe} elsewhere in Europe and ${c.global} in the rest of world seen, not listed`); }
    } catch (e) {
