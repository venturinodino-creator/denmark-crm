"""
tender_scraper.py — Daily scraper for Denmark tenders that align with an
Elsevier product
Source: TED Europa (EU official journal, filtered to place-of-performance =
Denmark). Denmark's national procurement portal (udbud.dk) has no verified
public JSON API equivalent to the Netherlands' TenderNed, so it is not
scraped here — only TED Europa, which as an EU member state already covers
Denmark's larger, EU-threshold procurement notices.
...
CATEGORY_KEYWORDS = [
    ...
    "videnskabelige databaser",   # DA: scientific/academic databases
    "forskningsinformation",       # DA: research information
    "litteraturdatabaser",         # DA: literature databases
]
...
PRODUCT_ALIGNMENT = {
    ...
    "forskningsinformation": "Elsevier Pure",
    ...
    "videnskabelige databaser": "Scopus",
    "litteraturdatabaser": "Scopus",
...
def write_notification(new_tenders: list) -> None:
    ...
    if not output_path or not new_tenders:
        return
    lines = [f"**{len(new_tenders)} new Denmark tender(s) competing with Elsevier found today.**\n"]
    ...
    # external, not fully trusted content. A fixed heredoc delimiter would
    # let a tender listing crafted to contain that exact line prematurely
    # close the block and inject arbitrary key=value pairs into
    # $GITHUB_OUTPUT (a known GitHub Actions workflow-command-injection
...
# Field names verified against the live API as of 2026-08-13 for the
# Netherlands version of this scraper — identical schema applies here, only
# the place-of-performance country code differs (DNK vs NLD).
...
def main():
    ...
    # TED Europa (only verified source for Denmark — see module docstring)
